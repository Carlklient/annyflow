...
const CARDS = [
  {
    ...
    from: { x: -90, y: 0 },
    delay: 0,
    /** Continuous drift — mostly horizontal, never idle */
    drift: { x: [-6, 8, -6], y: [0, -3, 0], duration: 5.5 },
  },
  {
    id: "sheets",
    ...
    from: { x: 90, y: 0 },
    delay: 0.12,
    drift: { x: [6, -8, 6], y: [0, 3, 0], duration: 6.2 },
  },
  {
    id: "dialer",
    ...
    from: { x: 0, y: 70 },
    delay: 0.24,
    drift: { x: [-4, 5, -4], y: [4, -5, 4], duration: 4.8 },
  },
] as const;
    ...
    <div className="absolute inset-x-[12%] top-[16%] bottom-[26%] z-10 grid grid-cols-4 items-end gap-0.5 p-1">
      {heights.map((h, i) => (
        <motion.div
          ...
          animate={{
            height: [`${Math.max(18, h - 28)}%`, `${h}%`, `${Math.max(22, h - 18)}%`],
          }}
          transition={{
            duration: 2.4 + i * 0.18,
            repeat: Infinity,
    ...
    <div className="relative w-full overflow-hidden border-t border-border bg-gradient-to-b from-white via-[#F8FAFC] to-[#F1F5F9]">
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-10 sm:px-8 sm:py-14 lg:px-10 lg:py-16">
        <div className="relative mx-auto aspect-[16/11] w-full max-w-5xl sm:aspect-[16/10]">
          {/* Flowing connectors — dash never stops */}
          <svg
          ...
          >
            {[
              ...
              <motion.path
                ...
                transition={{
                  pathLength: { duration: 1.1, delay: 0.7 + i * 0.08, ease: EASE },
                  opacity: { duration: 0.6, delay: 0.7 + i * 0.08 },
                  strokeDashoffset: {
                    duration: 2.2 + i * 0.25,
                    repeat: Infinity,
                    ease: "linear",
          ...
          {/* Main scene — enters once, then keeps a slow live drift */}
          <motion.div
          ...
          >
            <motion.div
              animate={{ y: [0, -5, 0, 4, 0], x: [0, 3, 0, -2, 0] }}
              transition={{
                y: { duration: 9, repeat: Infinity, ease: "easeInOut" },
                x: { duration: 11, repeat: Infinity, ease: "easeInOut" },
              }}
              className="relative overflow-hidden rounded-[1.5rem] border border-border bg-white shadow-[0_24px_80px_rgba(17,24,39,0.12)] sm:rounded-[1.75rem]"
            >
          ...
          {/* Floating cards — enter once, then never stop drifting */}
          {CARDS.map((card) => {
          ...
          <motion.div
            ...
            transition={{
              opacity: { delay: 0.95, duration: 0.4 },
          ...
          <motion.div
            ...
            transition={{
              ...
              x: { duration: 6.5, repeat: Infinity, ease: "easeInOut", delay: 1.3 },
            }}
